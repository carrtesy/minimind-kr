"""
# sglang 가속을 사용하려면, 먼저 아래 명령으로 (transformers 형식) 모델을 실행해야 합니다:
python -m sglang.launch_server --model-path ./minimind-kr-hf --attention-backend triton --host 0.0.0.0 --port 8998
"""
import os
import sys
...
class SGLangRolloutEngine(RolloutEngine):
    ...
    def rollout(self, prompt_ids: Tensor, attention_mask: Tensor, num_generations: int, max_new_tokens: int, temperature: float = 0.8) -> RolloutResult:
        # 왼쪽 padding 토큰을 제거하고, 유효한 토큰만 유지
        input_ids_list = []
...
def create_rollout_engine(
    ...
    if engine_type == "torch":
        ...
        raise ValueError(f"지원되지 않는 엔진 유형: {engine_type}")
