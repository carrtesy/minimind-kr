...
def dpo_loss(ref_log_probs, policy_log_probs, mask, beta):
    # ref_log_probs와 policy_log_probs 둘 다 shape: (batch_size, seq_len)
    ref_log_probs = (ref_log_probs * mask).sum(dim=1)
    policy_log_probs = (policy_log_probs * mask).sum(dim=1)

    # chosen과 rejected 데이터 분리
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MiniMind-KR DPO (Direct Preference Optimization)")
    parser.add_argument("--save_dir", type=str, default="../out", help="모델 저장 디렉터리")
    parser.add_argument('--save_weight', default='dpo', type=str, help="저장할 가중치의 접두사 이름")
    ...
    parser.add_argument('--max_seq_len', default=1024, type=int, help="학습 시 최대 절단 길이 (한국어 토큰당 문자 수는 tokenizer/데이터에 따라 다름)")
    parser.add_argument('--use_moe', default=0, type=int, choices=[0, 1], help="MoE 아키텍처 사용 여부 (0=아니오, 1=예)")
    parser.add_argument("--data_path", type=str, default="../dataset/dpo.jsonl", help="DPO 학습 데이터 경로")
    ...
    parser.add_argument("--wandb_project", type=str, default="MiniMind-DPO", help="wandb 프로젝트명")
