...
if __name__ == "__main__":
    # MoE 모델로 dense 모델을 증류하는 예시. 더 큰 teacher_hidden_size 모델로 더 작은 student_hidden_size 모델을 증류하는 것도 가능
    parser = argparse.ArgumentParser(description="MiniMind-KR 지식 증류")
    parser.add_argument("--save_dir", type=str, default="../out", help="모델 저장 디렉터리")
    parser.add_argument('--save_weight', default='full_dist', type=str, help="저장할 가중치의 접두사 이름")
    ...
    parser.add_argument("--max_seq_len", type=int, default=340, help="학습 시 최대 잘림 길이 (한국어 1 token ≈ 문맥에 따라 다름)")
    parser.add_argument("--data_path", type=str, default="../dataset/sft_t2t_mini.jsonl", help="학습 데이터 경로")
    ...
    parser.add_argument('--from_student_weight', default='full_sft', type=str, help="학생 모델이 어떤 가중치를 기반으로 할지")
    parser.add_argument('--from_teacher_weight', default='full_sft', type=str, help="교사 모델이 어떤 가중치를 기반으로 할지")
    ...
    parser.add_argument("--wandb_project", type=str, default="MiniMind-Distillation", help="wandb 프로젝트명")
