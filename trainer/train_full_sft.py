...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MiniMind-KR Full SFT")
    parser.add_argument("--save_dir", type=str, default="../out", help="모델 저장 디렉터리")
    parser.add_argument('--save_weight', default='full_sft', type=str, help="저장할 가중치의 접두사 이름")
    ...
    parser.add_argument('--max_seq_len', default=768, type=int, help="학습 시 최대 절단 길이 (한국어 토큰당 문자 수는 tokenizer/데이터에 따라 다름)")
    parser.add_argument('--use_moe', default=0, type=int, choices=[0, 1], help="MoE 아키텍처 사용 여부 (0=아니오, 1=예)")
    parser.add_argument('--seed', default=42, type=int, help="랜덤 시드 (DDP에서 각 rank는 seed+rank, 각 epoch은 seed+epoch)")
    parser.add_argument("--data_path", type=str, default="../dataset/sft_t2t_mini.jsonl", help="학습 데이터 경로")
    parser.add_argument('--from_weight', default='pretrain', type=str, help="어떤 가중치를 기반으로 학습할지. none인 경우 아무 가중치도 로드하지 않음")
    ...
    parser.add_argument("--wandb_project", type=str, default="MiniMind-Full-SFT", help="wandb 프로젝트명")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if skip > 0: 
            Logger(f'Epoch [{epoch + 1}/{args.epochs}]: 앞의 {start_step}개 step을 건너뜀, step {start_step + 1}부터 시작')
